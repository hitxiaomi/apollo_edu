...
Status OnLanePlanning::Init(const PlanningConfig& config) {
  ...
  AINFO << " planner config :" << config_.DebugString();
  if (!planner_) {
    return Status(
...
bool OnLanePlanning::DeadEndHandle(
  ...
  if (base_map_ptr->GetJunctions(dead_end_point, 1.0, &junctions) != 0) {
    AINFO << "Fail to get junctions from base_map.";
    return false;
  }
  if (junctions.size() <= 0) {
    AINFO << "No junction from map";
    return false;
  }
  Vec2d car_position;
  ...
  if (!JudgeCarInDeadEndJunction(&junctions, car_position, &junction)) {
    AINFO << "Target Dead End not found";
    return false;
  }
  return true;
}
...
void OnLanePlanning::RunOnce(const LocalView& local_view,
  ...
  // chassis
  AINFO << "Get chassis:" << local_view_.chassis->DebugString();
  ...
  if (frame_->open_space_info().is_on_open_space_trajectory()) {
    FillPlanningPb(start_timestamp, ptr_trajectory_pb);
    AINFO << "Planning pb:" << ptr_trajectory_pb->header().DebugString();
    frame_->set_current_frame_planned_trajectory(*ptr_trajectory_pb);
  } else {
    auto* ref_line_task =
...
Status OnLanePlanning::Plan(
  ...
  AINFO << "before planner plan :" <<  ptr_trajectory_pb->DebugString();
  AINFO << "planner name : " << planner_->Name();
  auto status = planner_->Plan(stitching_trajectory.back(), frame_.get(),
                               ptr_trajectory_pb);

  AINFO << "after planner plan :" << ptr_trajectory_pb->DebugString();
  ...
  if (frame_->open_space_info().is_on_open_space_trajectory()) {
    ...
    if (FLAGS_enable_record_debug) {
      ...
      AINFO << "Open space debug information added!";
    ...
    AINFO << "current_time_stamp: " << current_time_stamp;
